...
<template>
  <button v-on:keyup.enter="props.software!.selected = !props.software!.selected">
    <div
      @click="props.software!.selected = !props.software!.selected"
      :class="[props.software!.selected ? ['outline outline-4 outline-amber-400'] : ['']]"
      class="group flex flex-col pt-4 bg-gradient-to-t from-[#0F2027]/90 to-[#2C5364]/90 shadow-2xl backdrop-filter backdrop-blur-3xl rounded-xl hover:from-white/60 hover:to-white/20 w-[88px] h-[115px]"
    >
      <div>
        <img
          class="max-w-[44px] aspect-square m-auto"
          :src="getImageeUrl(props.software!.logo)"
          :alt="getImageAlt(props.software!.name)"
        />
        <h1
          :class="[size<=7 && props.software!.name.length<=15 ? ['text-lg'] : [`text-[15px] mt-[14px]`]]"
